...
/* 前向 linear 部分：每個 output neuron 獨立，可以平行化 */
#if defined(_OPENMP)
...
static void Layer_feedBack_full(Layer* self)
{
    ...
    /* OpenMP 版本：每個 thread 有一份私有的 errors_private[] */
    #pragma omp parallel
    {
        double *errors_private = (double*)calloc(in_size, sizeof(double));
        if (!errors_private) {
            /* 如果分配失敗，退回到單執行緒邏輯會比較安全，
               但作業情境下你也可以直接 assert 或 exit。*/
        }
        ...
        for (int i = 0; i < out_size; i++) {
            for (int j = 0; j < in_size; j++) {
                ...
                int    w_idx = i * in_size + j;
                double wij   = self->weights[w_idx];

                /* 對前一層的 error 累加：只加在自己的 private buffer */
                errors_private[j] += wij * dnet;

                /* 這個 index 對於 (i,j) 是唯一的，所以沒有 race */
                self->u_weights[w_idx] += dnet * lprev->outputs[j];

                /* 每個 i 只在該 iteration 更新一次，也沒有 race */
                self->u_biases[i] += dnet;
            }
        }

        /* 合併每個 thread 的 private errors -> shared lprev->errors */
        #pragma omp critical
        {
            for (int j = 0; j < in_size; j++) {
                lprev->errors[j] += errors_private[j];
            }
        }
